                   ...
                   unsigned int keybits)
{
    ...
    switch (keybits)
    {
        case 128: ctx->rounds = 10; break;
        case 192: ctx->rounds = 12; break;
        case 256: ctx->rounds = 14; break;
        default:
            return CRYPTO_ERR_AES_INVALID_KEY_SIZE;
    }
                   ...
                   unsigned int keybits)
{
    ...
    status = aes_setkey_enc(ctx, key, keybits);
    if (status < 0)
        return status;
...
#endif /* ?FULL_UNROLL */
//    AES_VALIDATE_RET(ctx != NULL);
//    AES_VALIDATE_RET(in  != NULL);
//    AES_VALIDATE_RET(out != NULL);
...
{
    size_t n;
    const unsigned  char *civ = iv;
    AES_VALIDATE_RET(iv != NULL);
    if (mode == CRYPTO_AES_ENCRYPT) {
        while (length) {
            for (n = 0; n < 16 && n < length; ++n)
                output[n] = input[n] ^ civ[n];
            for (; n < 16; ++n) {
                output[n] = civ[n];
            }
            aes_encrypt(output, output, ctx);
            civ = output;
            if (length < 16)
                break;
            length -= 16;
        ...
        while (length) {
            unsigned char c;
            aes_decrypt(input, tmp.c, ctx);
            for (n = 0; n < 16 && n < length; ++n) {
                c = input[n];
                output[n] = tmp.c[n] ^ iv[n];
                iv[n] = c;
            }
            if (length <= 16) {
                for (; n < 16; ++n)
                     ...
                     unsigned char *output)
{
    ...
    while (l < length) {
        if (n == 0) {
            aes_encrypt(iv, ecount_buf, ctx);
            ctr128_inc(iv);
        }
        output[l] = input[l] ^ ecount_buf[n];
        ++l;
        n = (n + 1) % 16;
    }
    *num = n;
